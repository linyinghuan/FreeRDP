...
void auditor_rdpdr_update_path_table(proxyData* pData, AUDITOR_RDPDR_PATH_TABLE_HEAD* table, char* key, AUDITOR_RDPDR_PATH_LIST_NODE* list)
{
	...
	if(NULL == pOldList) {
		...
		while(pNext) {
			...
			auditor_file_event_produce(AUDITOR_EVENT_TYPE_CLIPBOARD_UPLOAD, pData->ps->uuid, pNext->path->m_path, pNext->path->size, file_pos);
			pNext = pNext->next;
		}
	} else {
		pNext = list;
...
void auditor_rdpdr_server_event_handler(proxyData* pData, proxyChannelDataEventInfo* pEvent, AUDITOR_CTX_DATA *auditor_ctx)
{
	...
	if (auditor_ctx->g_createNewFileNeed) {
		...
		if (IoStatus == 0) {
			...
			auditor_file_event_produce(AUDITOR_EVENT_TYPE_CLIPBOARD_DOWNLOAD, pData->ps->uuid, auditor_ctx->g_createNewFilePath, 0, file_pos);
		}
	}
